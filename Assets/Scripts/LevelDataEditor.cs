...
public class LevelDataEditor : Editor 
{
	...
	private void OnEnable() 
	{
		...
		list.onAddDropdownCallback = (Rect buttonRect, ReorderableList l) => {
			...
			guids = AssetDatabase.FindAssets("", new[]{"Assets/Prefabs/Kokoko"});
			foreach (var guid in guids) {
				var path = AssetDatabase.GUIDToAssetPath(guid);
				menu.AddItem(new GUIContent("Kokoko/" + Path.GetFileNameWithoutExtension(path)), false, clickHandler, new WaveCreationParams() {Type = MobWave.WaveType.Boss, Path = path});
			}
			menu.ShowAsContext();
		};
	}
