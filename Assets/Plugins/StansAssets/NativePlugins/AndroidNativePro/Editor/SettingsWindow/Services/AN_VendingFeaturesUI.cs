...
namespace SA.Android.Editor
{
    ...
    {
        readonly GUIContent m_ProductIdDLabel = new GUIContent("Product Id[?]:", "A unique identifier that will be used for reporting. It can be composed of letters and numbers.");
        readonly GUIContent m_DisplayNameLabel = new GUIContent("Display Name[?]:", "This is the name of the Google Product that will be seen by customers (if this is their primary language). For automatically renewable subscriptions, don’t include a duration in the display name. The display name can’t be longer than 75 characters.");
        ...
        readonly GUIContent m_ConsumableLabel = new GUIContent("Consumable[?]:", "Indicates if product can be consumed after purchase");
        readonly GUIContent m_ProductType = new GUIContent("ProductType[?]:", "Defines the product type");
        readonly GUIContent m_PriceLabel = new GUIContent("Price[?]:", "The price of the product");
        ...
        protected override void OnServiceUI()
        {
            ...
            using (new SA_WindowBlockWithSpace(new GUIContent("Google Play Billing ")))
            {
                ...
                if (AN_Settings.Instance.GooglePlayBilling)
                {
                    ...
                    IMGUILayout.ReorderablList(AN_Settings.Instance.InAppProducts, GetProductDisplayName, DrawProductContent, () =>
                    {
        ...
        {
            product.Sku = IMGUILayout.TextField(m_ProductIdDLabel, product.Sku);
            product.Title = IMGUILayout.TextField(m_DisplayNameLabel, product.Title);
            product.Type = (AN_BillingClient.SkuType)IMGUILayout.EnumPopup(m_ProductType, product.Type);
            product.IsConsumable = IMGUILayout.ToggleFiled(m_ConsumableLabel, product.IsConsumable, IMGUIToggleStyle.ToggleType.YesNo);
            product.Price = IMGUILayout.TextField(m_PriceLabel, product.Price);
